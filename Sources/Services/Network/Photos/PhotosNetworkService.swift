//
//  PhotoCommentsNetworkService.swift
//  STPhotoDetails
//
...
class PhotosNetworkService: PhotosServiceProtocol {
    ...
    func fetchPhotos(photoIds: [String]?, entityFilter: GetPhotosOperationModel.EntityFilter?, completionHandler: @escaping (Result<[STPhoto], OperationError>) -> Void) {
        let operation = GetPhotosOperation(model: GetPhotosOperationModel.Request(photoIds: photoIds, entityFilter: entityFilter)) { (result) in
            switch result {
